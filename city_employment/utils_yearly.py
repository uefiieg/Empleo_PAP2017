...
def data_clean(data, sectores=False):
    ...
    for column in special_columns:
        data[column] = data[column].apply(lambda x: x.decode('utf8'))
        data[column] = data[column].apply(text_clean)
...
def lag_definition(data, city, default_lags=1):
...
def dataframe_model(data, city, final_lags, sectors_df):
    if data is None or final_lags is None:
        return None

    city_index = data.loc[data.municipio == city].index[0]
    split_date = data.columns[2].split('_')
    month_nb = datetime.datetime.strptime(transform_spanish_months(str(split_date[1])),
                                          '%B').month
    min_date = datetime.datetime(int(split_date[0]), month_nb, 1)
    min_month_row = min_date + datetime.timedelta(days=365 * max(final_lags))
    index_to_begin = str(min_month_row.year) + '_' + str('diciembre')

    column_to_begin = [pos for pos in range(len(data.columns)) if data.columns[pos] == index_to_begin][0]
    ...
    for index in range(len(df.index)):
        ...
        # period_growth.append(df.iloc[index].values[0] / df.iloc[index-1].values-1)
